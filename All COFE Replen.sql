...
LEFT OUTER JOIN(SELECT item_number, COUNT(item_number) AS 'total' FROM t_stored_item with(NOLOCK) WHERE type = '0' AND (location_id LIKE 'AA%' OR location_id LIKE 'AB%' OR location_id LIKE 'AC%') AND status = 'A' GROUP BY item_number) t
